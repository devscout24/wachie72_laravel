...
class UserAuthBDController extends Controller
{
    ...
    {
        // Map confirmed_password to password_confirmation first
        $request->merge([
            'password_confirmation' => $request->confirmed_password ?? null,
        ]);
        ...
        DB::beginTransaction();
        try {
            $data = $request->only(['name', 'surname', 'email', 'password', 'username', 'phone', 'country', 'city']);
            $data['password'] = Hash::make($data['password']);
            $data['username'] = $data['username'] ?? $this->generateUniqueUsername($request->name);
